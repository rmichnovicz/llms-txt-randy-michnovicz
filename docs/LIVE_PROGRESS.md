...
The worker supervises a bounded pool of child threads in both embedded and standalone modes. `BRIEF_WORKER_CONCURRENCY` defaults to `4`; each child claims one job at a time and runs its own event loop and lease heartbeat. A blocked child does not block another child's renewals. Shutdown cancels each child's active work and heartbeat before joining the threads; already-running synchronous calls can still finish. Set concurrency to `1` for serial processing.
